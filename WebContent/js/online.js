...
! function() {
		...
		ajax: function(conf) {
			...
			if(xhr) {
				...
				xhr.onreadystatechange = function() {
					if(4 == xhr.readyState)
						if(200 === xhr.status) success(eval("(" + xhr.responseText + ")"));
						else error()
				};
	...
	ysf.on = function() {
		...
		return function(e) {
			...
			if(/object/gi.test(t)) {
				for(var n in e)
					if("onload" == n && util.isFunction(e[n])) util.addEvent(proxy, n, e[n]);
...
var __YSFOPTION__ = {
	...
	hidden: 0,
	uid: window.YSFUID || '',
	appKey: '60b6cb9a23360f7292805f1aefa9a35a'
};
try {
